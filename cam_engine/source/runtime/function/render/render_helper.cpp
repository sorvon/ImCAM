...
namespace ImCAM
{
    ...
    glm::mat4 CalculateDirectionalLightCamera(RenderScene& scene, RenderCamera& camera)
    {
        ...
        // CascadedShadowMaps11 / CreateFrustumPointsFromCascadeInterval
        {
            ...
            frustum_bounding_box.min_bound = glm::vec3(FLT_MAX, FLT_MAX, FLT_MAX);
            frustum_bounding_box.max_bound = glm::vec3(FLT_MIN, FLT_MIN, FLT_MIN);
            ...
            for (size_t i = 0; i < CORNER_COUNT; ++i)
            {
                glm::vec4 frustum_point_with_w = inverse_proj_view_matrix * glm::vec4(g_frustum_points_ndc_space[i].x,
                                                                                      ...
                                                                                      1.0);
                glm::vec3   frustum_point        = glm::vec3(frustum_point_with_w.x / frustum_point_with_w.w,
        ...
        glm::mat4 light_proj;
        {
            ...
            glm::vec3 eye =
                box_center + scene.m_directional_light.m_direction * glm::length(box_extents);
